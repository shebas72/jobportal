<?php
    ...
    if(($_SESSION['email'])&&($_SESSION['password']))
    {
        ...
        $sqljobcheck = "SELECT * FROM company WHERE email=\"$email\" AND password=\"$password\";";
        ...
        if($action=='reject')
        {                
            $sql="UPDATE applicants SET status=\"Rejected\" WHERE jid=\"$jid\" AND uid=\"$uid\";";
            $res=mysql_query($sql);
            $sql="INSERT into messages(cid,jid,uid,status) VALUES(\"$cid\",\"$jid\",\"$uid\",0);";
            $res=mysql_query($sql);
        }
        
        if($action=='unreject')
        {                
            $sql="UPDATE applicants SET status=\"Shortlisted\" WHERE jid=\"$jid\" AND uid=\"$uid\";";
            $res=mysql_query($sql);
            $sql="INSERT into messages(cid,jid,uid,status) VALUES(\"$cid\",\"$jid\",\"$uid\",1);";
            $res=mysql_query($sql);
        }        
                            ...
                            <div class="container">
                                <div class="container-sm-height">
                                    <div class="row-sm-height">
                                        <div class="col-xs-12 col-sm-height btn btn-transparent col-sm-2" href="company-job-applied.php?jid=<?php echo $jid; ?>" onclick="location.href='company-job-applied.php?jid=<?php echo $jid; ?>'">
                                            Applied
                                            <?php
                                        ...
                                        <div class="col-xs-12 col-sm-height btn btn-transparent col-sm-2 active" href="company-job-rejected.php?jid=<?php echo $jid; ?>" onclick="location.href='company-job-rejected.php?jid=<?php echo $jid; ?>'">
                                            Rejected
                                            <?php
                                ...
                                <div class="col-xs-12">
                                    <div ui-view="">
                                        <div>
                                            <div class="pull-right found-count">
                                                <div ng-switch="vj.tab">                                            
                                                    <div ng-switch-when="matched"> <?php echo("$countrr"); ?> Matches </div>
                                                </div>
                                            </div>
                                            <div class="clearfix"></div>
                                        </div>
                                        <ul class="people-list-big">
                                            ...
                                            <?php
                                                $sqlf = mysql_query("SELECT * FROM applicants WHERE jid='$jid' AND status=\"Rejected\"");
                                                while($rowf = mysql_fetch_array($sqlf))
                                                {
                                                    $uid = $rowf['uid'];
                                                    $appliedon = $rowf['date_updated'];
                                                    $sqlck = "SELECT * FROM candidate WHERE id=\"$uid\";";
                                                    $resck=mysql_query($sqlck);                                                
                                                    while($rowu = mysql_fetch_array($resck)) 
                                                    {
                                                        $fname = $rowu['fname'];
                                                        $lname = $rowu['lname'];
                                                ...
                                                ?>
                                                <li class="people-list-big-item" current-page="vj.pagination.current" total-items="vj.totalItems.count" ng-if="(!vj.isRejectedTab && application.status != 'rejected') || (vj.isRejectedTab && application.status != 'live')" ng-repeat="application in vj.items.applications | itemsPerPage: vj.itemsPerPage">
